...
public class PossibleBipartition {
	...
	public static void main(String[] args) {
		PossibleBipartition o = new PossibleBipartition();
		System.out.println(o.possibleBipartition(5, new int[][] { { 1, 2 }, { 2, 3 }, { 3, 4 }, { 4, 5 }, { 1, 5 } }));
		System.out.println(o.possibleBipartition(3, new int[][] { { 1, 2 }, { 1, 3 }, { 2, 3 } }));
		System.out.println(o.possibleBipartition(4, new int[][] { { 1, 2 }, { 1, 3 }, { 2, 4 } }));
		System.out.println(o.possibleBipartition(50, new int[][] { { 21, 47 }, { 4, 41 }, { 2, 41 }, { 36, 42 },
	...
	public boolean possibleBipartition(int N, int[][] dislikes) {
		...
		for (int[] dislike : dislikes)
			g.addEdge(dislike[0], dislike[1]);

		Colour[] colours = new Colour[N + 1];
		for (int u = 1; u <= N; u++) {
			if (colours[u] == null) {
				if (!isBipartite(g, u, colours, Colour.BLACK))
					return false;
			}
		}
		return true;
	...
	private boolean isBipartite(Graph g, int u, Colour[] colours, Colour colour) {
		colours[u] = colour;
		Colour neighbourColor = colour == Colour.BLACK ? Colour.WHITE : Colour.BLACK;
		for (int neighbour : g.adj[u]) {
			if (colours[neighbour] == null) {
				if (!isBipartite(g, neighbour, colours, neighbourColor))
					return false;
